...
const VESTAS_CIF = companyConfig.id;
...
describe('Integration: API Workflow', () => {
  ...
  describe('ANAF API', () => {
    ...
    itIfAnaf('should use cached data when API fails (getCompanyFromANAFWithFallback)', async () => {
      const cached = { cui: 23012802, name: COMPANY_NAME };
  ...
  describe('API Jobs Core', () => {
    ...
    itIfApi('should query jobs by CIF and return valid data', async () => {
      ...
      if (result.numFound === 0) {
        console.log('⚠️ No VESTAS jobs in API — skipping job field assertions (scraper may not have run yet)');
        return;
      }
  ...
  describe('Full Validation Workflow', () => {
    ...
    itIfAnaf('should complete the ANAF → Peviitor validation path', async () => {
      ...
      const vestasCompany = searchResults.find(c =>
        c.cui.toString() === COMPANY_CIF && c.statusLabel === 'Funcțiune'
      );
      expect(vestasCompany).toBeDefined();

      const anafData = await anaf.getCompanyFromANAF(vestasCompany.cui.toString());
    ...
    itIfApi('should validate company and query API for existing jobs', async () => {
      ...
      if (companyResult.existingJobsCount === 0) {
        console.log('⚠️ No VESTAS jobs in API — skipping job count assertion (scraper may not have run yet)');
        return;
      }
