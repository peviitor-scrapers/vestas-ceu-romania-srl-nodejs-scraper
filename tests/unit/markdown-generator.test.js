...
const baseCompany = {
  ...
  career: ["https://careers.vestas.com"],
  lastScraped: "2026-08-07"
};

const baseJob = {
  url: "https://careers.vestas.com/en/vacancy/123_en",
...
describe("generateJobsMarkdown", () => {
  describe("company section", () => {
    it("includes company name as h1", () => {
      ...
      expect(md).toContain("# VESTAS CEU ROMANIA S.R.L.");
    });

    it("includes CIF", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("23012802");
    });

    it("includes brand", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("VESTAS");
    });
    ...
    it("includes website as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://www.vestas.com](https://www.vestas.com)");
    });

    it("includes career page as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://careers.vestas.com](https://careers.vestas.com)");
    });
    ...
    it("omits optional fields when not present", () => {
      const minimal = { id: "23012802", company: "VESTAS CEU ROMANIA S.R.L." };
  ...
  describe("jobs section", () => {
    ...
    it("includes job URL as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, [baseJob]);
      expect(md).toContain("[https://careers.vestas.com/en/vacancy/123_en]");
    });
    ...
    it("renders multiple jobs", () => {
      const job2 = { ...baseJob, title: "DevOps Engineer", url: "https://careers.vestas.com/en/vacancy/456_en" };
    ...
    it("handles job with no optional fields", () => {
      const minimal = { url: "https://careers.vestas.com/en/vacancy/999_en", title: "QA Engineer" };
